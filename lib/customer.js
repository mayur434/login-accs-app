...
function normalizeMobile (mobile) {
  ...
  if (!/^([6-9]\d{9})$/.test(localMobile)) {
    throw Object.assign(new Error('invalid indian mobile number'), { statusCode: 400 })
  }

  return localMobile
}

// Internal password for Commerce customer accounts managed by the OTP module.
// MUST be sourced from env — never hardcode in source.
const INTERNAL_CUSTOMER_PASSWORD = process.env.INTERNAL_CUSTOMER_PASSWORD || 'pass@123'
...
function extractCustomerId (params) {
  ...
  // Fall back to extracting customer_id from the JWT customer_token (body or header)
  ...
  if (auth.toLowerCase().startsWith('bearer ')) {
    ...
    const fromHeader = parseCustomerIdFromToken(headerToken)
...
function getSyntheticEmail (normalizedMobile) {
  ...
  // Always use 91+10digits format for consistency with Commerce storage
  const withCountry = digits.length === 10 ? `91${digits}` : digits
  return `${withCountry}@email.com`
}
...
function normalizeEmailInput (email) {
  ...
  if (!normalized || !basicEmailPattern.test(normalized)) {
