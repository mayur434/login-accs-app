...
const FETCH_TIMEOUT_MS = Number(process.env.GRAPHQL_TIMEOUT_MS) || 10000 // 10s default
...
async function _doRequest (endpoint, query, variables, logger, authToken, extraHeaders = {}) {
  ...
  logger.debug?.(`calling GraphQL ${endpoint}`)
  ...
  try {
    ...
    if (err.name === 'AbortError') {
      throw new Error(`Commerce GraphQL timeout (${FETCH_TIMEOUT_MS}ms) for ${endpoint}`)
    }
    throw err
  } finally {
    clearTimeout(timer)
