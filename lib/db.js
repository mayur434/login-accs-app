...
const { dbStart, dbEnd, setPerfLogging } = require('./logger')

const APP_CONFIG_ID = 'app_config'
const APP_CONFIG_COLLECTION = 'app_config'
const STATE_CONFIG_KEY = 'app_config_cache'
const STATE_CONFIG_TTL_SECONDS = Number(process.env.STATE_CONFIG_TTL_SECONDS)

// Lightweight flag for perf logging check (avoids async state call in hot path)
let _perfLoggingEnabled = false

function invalidateAppConfigCache () {
  _perfLoggingEnabled = false
  invalidateStateConfigCache()
}
 ...
 * Only active when perf_logging is enabled in app_config (toggleable at runtime).
 */
function wrapCollectionWithLogging (rawCollection, collectionName, rawDbClient, parentTraceId) {
  ...
  function wrap (opName) {
    return async function (...args) {
      // Check perf_logging flag — if off, skip logging overhead
      if (!_perfLoggingEnabled) {
        return rawCollection[opName](...args)
      }
...
const APP_CONFIG_DEFAULTS = {
  ...
  // Performance logging (toggleable at runtime via config API)
  perf_logging: false
}
...
function normalizeAppConfig (config) {
  ...
  return {
    ...
    // Performance logging
    perf_logging: bool('perf_logging')
  }
}

// ── aio-lib-state config cache (survives cold starts) ───────────────────
...
async function getStateClient () {
  if (!_stateClient) {
    ...
    try {
      ...
      // Don't cache null — retry on next call
      return null
    }
  }
  return _stateClient
...
/**
 * Try reading app_config from aio-lib-state (fast, no DB connection).
 * Returns null on miss or error.
 */
async function getConfigFromState () {
  try {
    const state = await getStateClient()
    const res = await state.get(STATE_CONFIG_KEY)
...
/**
 * Write app_config to aio-lib-state with TTL (5 days default).
 */
async function putConfigToState (config) {
  try {
    const state = await getStateClient()
    await state.put(STATE_CONFIG_KEY, config, { ttl: STATE_CONFIG_TTL_SECONDS })
  } catch (_) { /* non-critical — best-effort cache */ }
}

/**
 * Invalidate aio-lib-state config cache.
 */
async function invalidateStateConfigCache () {
  try {
    const state = await getStateClient()
    await state.delete(STATE_CONFIG_KEY)
  } catch (_) { /* ignore */ }
}

/**
 * Read app_config: state (primary) → DB (fallback on miss) → warm state.
 * aio-lib-state is the single cache layer. No in-memory caching.
 */
async function getAppConfig (dbClient) {
  // 1. aio-lib-state (primary — survives cold starts, shared across containers)
  const stateConfig = await getConfigFromState()
  if (stateConfig) {
    _perfLoggingEnabled = !!stateConfig.perf_logging; setPerfLogging(_perfLoggingEnabled)
    return stateConfig
  }

  // 2. DB fallback — state miss, read from source of truth
  ...
  _perfLoggingEnabled = !!normalized.perf_logging; setPerfLogging(_perfLoggingEnabled)
  // Warm state so next call skips DB entirely
