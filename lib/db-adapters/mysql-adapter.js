...
// Module-level flag: migrateCriticalColumns runs at most once per container lifecycle
...
async function connect (params) {
  ...
  // Run schema migration only once per container lifecycle (not every request)
  if (!_migrationDone) {
...
async function migrateCriticalColumns (pool) {
  ...
  await ensureColumn(pool, 'app_config', 'perf_logging', 'TINYINT(1) DEFAULT 0')
...
const TABLE_SCHEMAS = {
  app_config: `CREATE TABLE IF NOT EXISTS app_config (
    ...
    perf_logging TINYINT(1) DEFAULT 0,
