 ...
 *
 * IMPORTANT: All logging is gated by _perfLoggingEnabled flag.
 * At production scale (millions of requests), logging is OFF by default.
 * Enable via app_config { perf_logging: true } when debugging.
 */
...
// Controlled by db.js getAppConfig — only true when perf_logging is enabled
let _perfLoggingEnabled = false
function setPerfLogging (enabled) { _perfLoggingEnabled = !!enabled }

function generateTraceId () {
  return _perfLoggingEnabled ? crypto.randomUUID() : 'noop'
}
 ...
 * NOOP when perf_logging is disabled (production default).
 */
async function _persist (rawDbClient, row) {
  if (!_perfLoggingEnabled || !rawDbClient) return
  try {
