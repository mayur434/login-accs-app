...
const { serverError, badRequest, unauthorized, notFound } = require('../lib/http')
...
async function generateCommerceToken (email, params, logger) {
  ...
  try {
    const resp = await commerceGraphQLRequest(params, mutation, { email }, logger)
...
async function upsertGoogleIdentity (collection, customerId, email, firstname, lastname, googleSub, logger) {
  const now = new Date()
  // Always use email as the upsert filter — it is stable and always present.
  // customer_id may be null when the Commerce customer pre-existed (create skipped).
  const setFields = {
    email,
  ...
  if (customerId != null) setFields.customer_id = customerId
  if (firstname) setFields.firstname = firstname
  if (lastname) setFields.lastname = lastname
  if (googleSub) setFields.google_sub = googleSub
  ...
  try {
    ...
    logger.info(`Google SSO identity upserted for email=${email}, customer_id=${customerId}`)
  } catch (e) {
  ...
  try {
    ...
    if (existing) {
      ...
      if (!token) {
        ...
        return notFound('user exists in identity DB but not found in Commerce')
      }
      ...
      // Always refresh: backfill google_sub if missing, update customer_id if resolved, touch updated_at
      const resolvedId = parseCustomerIdValue(existing.customer_id) || parseCustomerIdValue(profile?.id) || null
      await upsertGoogleIdentity(collection, resolvedId, normalizedEmail, firstname, lastname, googleSub, logger)
    ...
    // Fetch profile first — gives us the authoritative customer id from Commerce
    let profile = null
    try { profile = await fetchCustomerProfile(inParams, token, logger) } catch (e) {
      logger.debug('Could not fetch customer profile after registration: ' + e.message)
    }

    // Resolve customer_id: profile.id > create response id > token payload fallback
    let customerId = parseCustomerIdValue(profile?.id)
    if (!customerId && createdCustomer?.id) {
      customerId = parseCustomerIdValue(createdCustomer.id)
    }
    if (!customerId) {
      try {
    ...
    logger.info(`New user customer_id resolved: ${customerId}`)
